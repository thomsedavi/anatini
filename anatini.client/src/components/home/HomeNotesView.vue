<script setup lang="ts">
  ...
  function test(label: string, note: Note): void {
    if (label === 'Bookmark') {
      const statusActions: StatusActions = {
      }
      ...
      if (note.channelHeader !== null) {
        apiFetchAuthenticated(`channels/${note.channelHeader.handle}/notes/${note.handle}/bookmark`, statusActions, init);
      } else if (note.userHeader !== null) {
        apiFetchAuthenticated(`users/${note.userHeader.handle}/notes/${note.handle}/bookmark`, statusActions, init);
      }
    }
  }
</script>
