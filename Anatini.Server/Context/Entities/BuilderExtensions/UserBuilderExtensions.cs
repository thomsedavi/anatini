...
namespace Anatini.Server.Context.Entities.BuilderExtensions
{
    public static class UserBuilderExtensions
    {
        public static void Configure(this EntityTypeBuilder<ApplicationUser> userBuilder)
        {
            ...
            userBuilder.Property(user => user.SecurityStamp).Has(order: 9);
            userBuilder.Property(user => user.TwoFactorEnabled).Has(order: 10);
            userBuilder.Property(user => user.LockoutEnd).Has(order: 11);
            userBuilder.Property(user => user.LockoutEnabled).Has(order: 12);
            userBuilder.Property(user => user.AccessFailedCount).Has(order: 13);
            ...
            userBuilder.HasIndex(user => user.Handle).IsUnique();
